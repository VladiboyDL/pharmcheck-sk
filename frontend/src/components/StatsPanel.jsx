  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {cards.map((card) => (
        <div
          key={card.label}
          className="bg-panel rounded-sm2 border border-hairline p-4 "
        >
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-sm2 bg-brand/15 ${colorMap[card.color]}  flex items-center justify-center`}>
              <StatIcon icon={card.icon} />
            </div>
            <div>
              <div className="text-xl font-bold text-txt">{card.value}</div>
              <div className="text-[11px] text-txt3 leading-tight">{card.label}</div>
            </div>
          </div>
        </div>
      ))}
...
function StatIcon({ icon }) {
  ...
  if (icon === "warning") {
    return (
      <svg className="w-5 h-5 text-txt" fill="currentColor" viewBox="0 0 24 24">
