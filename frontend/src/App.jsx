...
// Every surface is the brand ground now; the light theme is gone.
const DARK_TABS = new Set(["kiosk", "dispense", "impact", "checker", "atc", "profiles"]);
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors ${dark ? "bg-ink" : "bg-ink"}`}>
      {/* Header */}
      <header
        className={`sticky top-0 z-40 border-b transition-colors ${
          dark ? "bg-panel border-hairline" : "bg-panel border-hairline"
        }`}
      >
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 bg-brand rounded-sm2 flex items-center justify-center ">
                <svg className="w-5 h-5 text-txt" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
              </div>
              <div>
                <h1 className={`text-lg font-bold leading-tight ${dark ? "text-txt" : "text-txt"}`}>
                  AvatarAI <span className="font-normal text-txt2">Dispense</span>
                </h1>
                <p className="text-[11px] text-txt2 leading-tight">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-[10px] px-2.5 py-1 rounded-full font-semibold border ${
                  dark
                    ? "bg-surface2 text-txt2 border-hairline2"
          ...
          {/* Navigation tabs */}
          {/* Six tabs do not fit a phone; without this the last three are unreachable. */}
          <nav className="flex gap-1 -mb-px overflow-x-auto scrollbar-none -mx-4 px-4">
            {TABS.map((tab) => (
              <button
                key={tab.id}
                ...
                className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors whitespace-nowrap flex-shrink-0 ${
                  activeTab === tab.id
                    ? dark
                      ...
                      : "border-brand text-brand"
                    : dark
                    ? "border-transparent text-txt3 hover:text-txt2 hover:border-hairline2"
                    : "border-transparent text-txt3 hover:text-txt3 hover:border-hairline2"
                }`}
              >
      ...
      {/* Footer */}
      <footer className={`border-t mt-16 ${dark ? "border-hairline bg-ink" : "border-hairline bg-panel"}`}>
        <div className="max-w-6xl mx-auto px-4 py-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 text-xs text-txt2">
              ...
              <span className="text-warn font-medium">Nie je určený na klinické rozhodovanie</span>
            </div>
          </div>
        </div>
      </footer>
      ...
      {!results ? (
        <>
          ...
          {medications.length >= 2 && (
            <div className="flex items-center justify-between bg-panel rounded-sm2 border border-hairline p-4">
              <div className="text-sm text-txt3">
                <span className="font-semibold text-txt">{medications.length}</span> liekov vybraných &middot;{" "}
                <span className="font-semibold text-txt">{medications.length * (medications.length - 1) / 2}</span> párov na kontrolu
              </div>
              <button
                onClick={onCheck}
                disabled={loading}
                className="bg-brand hover:bg-brandDeep disabled:from-blue-400 disabled:to-indigo-400 text-txt font-semibold px-6 py-2.5 rounded-sm2 hover: transition-all flex items-center gap-2"
              >
                {loading ? (
                  <>
          ...
          {medications.length === 1 && (
            <p className="text-center text-sm text-txt2 py-2">
              Pridajte aspoň 2 lieky pre kontrolu interakcií
            </p>
          )}

          {error && (
            <div className="bg-surface border border-bad/40 text-bad rounded-sm2 p-4 text-sm flex items-center gap-3">
...
function DemoScenarios({ onLoad }) {
  ...
  return (
    <div className="mt-8 pt-6 border-t border-hairline">
      <h3 className="text-xs font-semibold text-txt2 uppercase tracking-wider text-center mb-4">
        Skúšobné scenáre
      </h3>
      <div className="grid gap-3 md:grid-cols-3">
        {scenarios.map((s) => (
          <button
            ...
            className="text-left bg-panel border border-hairline rounded-sm2 p-4 hover:border-brand hover: transition-all group"
          >
            <div className="flex items-center gap-2 mb-2">
              <ScenarioIcon icon={s.icon} />
              <div>
                <span className="font-semibold text-slate-800 group-hover:text-brand transition-colors text-sm">
                  {s.title}
                </span>
                <span className="text-[10px] bg-surface2 text-txt3 px-2 py-0.5 rounded-full ml-2">
                  {s.subtitle}
                </span>
              </div>
            </div>
            <p className="text-xs text-txt2">{s.description}</p>
            <div className="mt-2 text-[10px] text-brand font-medium opacity-0 group-hover:opacity-100 transition-opacity">
              Načítať scenár &rarr;
            </div>
          </button>
        ))}
...
function ScenarioIcon({ icon }) {
  if (icon === "elderly") {
    return (
      <div className="w-7 h-7 bg-surface rounded-sm2 flex items-center justify-center">
  ...
  if (icon === "brain") {
    return (
      <div className="w-7 h-7 bg-brand/15 rounded-sm2 flex items-center justify-center">
  ...
  return (
    <div className="w-7 h-7 bg-brand rounded-sm2 flex items-center justify-center">
      <svg className="w-4 h-4 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24">
